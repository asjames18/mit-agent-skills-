...
Python CLIs attach credentials through `bin/cred.py`: `from cred import add_surrogate_to_request, read_json_response`. On the original runtime this uses the secure vault (single-use surrogate values, attached per the connector's placement). Anywhere else, set `MIT_SKILLS_CUSTOM_CLOUDFLARE` and the value is sent as `Authorization: Bearer`. Authenticated requests must only go to the hosts below.
